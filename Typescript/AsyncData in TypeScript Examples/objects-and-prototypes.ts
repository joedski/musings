...
interface AsyncDataBase<ResultT, ErrorT> {
  // TODO: Add `this` types to all the methods to remove parametrization from the interface itself.
  cata<N, W, E, R>(caseMap: AsyncDataCaseMap<RT, ET, N, W, E, R>): (N | W | E | R);
  map<RTb>(fn: (a: ResultT) => RTb): AsyncData<RTb, ErrorT>;
  error<ETb>(fn: (e: ErrorT) => ETb): AsyncData<ResultT, ETb>;
  unnest<Rin, ETin>(this: AsyncData<AsyncData<Rin, ETin>, ErrorT>): AsyncData<Rin, ETin | ErrorT>;
  flatMap<RTb, ETb>(fn: (a: ResultT) => AsyncData<RTb, ETb>): AsyncData<RTb, ErrorT | ETb>;
}

// a prop named `status` is used because JS does not have tagged types.
export type AsyncData<ResultT, ErrorT> =
  AsyncDataBase<ResultT, ErrorT> & (
...
const AsyncDataPrototype: AsyncDataBase = {
  cata(caseMap) {
    if (this.status === Status.NotAsked) return caseMap.NotAsked();
    if (this.status === Status.Waiting) return caseMap.Waiting();
    if (this.status === Status.Error) return caseMap.Error(this.error);
    if (this.status === Status.Result) return caseMap.Result(this.result);
  },

  map(fn) {
    return this.cata({
      [Status.NotAsked]: () => this,
      [Status.Waiting]: () => this,
      [Status.Error]: () => this,
      [Status.Result]: (result) => AsyncData.Result(fn(result)),
    });
  },
  ...
  mapError(fn) {
    return this.cata({
      [Status.NotAsked]: () => this,
      [Status.Waiting]: () => this,
      [Status.Error]: (error) => AsyncData.Error(fn(error)),
      [Status.Result]: () => this,
    });
  },
